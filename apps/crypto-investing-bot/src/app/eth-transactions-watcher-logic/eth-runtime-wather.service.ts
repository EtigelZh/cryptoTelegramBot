...
export class EthRuntimeWatcherService implements OnModuleInit, OnModuleDestroy {
  ...
  private async _findDexTransactions(
    ...
    const targetWalletAddresses =
      mode == WathcingTransactionsMode.PRODUCTION
        ? this._targetWalletAddresses
        : [
            {
              alias: 'emulate',
              hash: walletHashes[0],
              walletSubscriptionMessages: emulationMessageContext,
            },
          ];
  ...
  private async _handleSwap(
    walletEntity: Pick<
      WalletEntity,
      'alias' | 'hash' | 'walletSubscriptionMessages'
    >,
    log: Log
  ) {
    ...
    const { action, tokenAddress } = economics;
